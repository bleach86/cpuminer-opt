...
static __thread alliumV2_ctx_holder alliumV2_ctx;

bool init_alliumV2_ctx()
{
        sph_keccak256_init( &alliumV2_ctx.keccak );
        cubehashInit( &alliumV2_ctx.cube, 256, 16, 32 );
        sph_skein256_init( &alliumV2_ctx.skein );
#if defined (__AES__)
        init_groestl256( &alliumV2_ctx.groestl, 32 );
#else
        sph_groestl256_init( &alliumV2_ctx.groestl );
#endif
        return true;
}
...
void alliumV2_hash(void *state, const void *input)
{
    ...
    alliumV2_ctx_holder ctx __attribute__ ((aligned (32)));

    memcpy( &ctx, &alliumV2_ctx, sizeof(alliumV2_ctx) );
                     ...
                     uint64_t *hashes_done )
{
    ...
    sph_blake256_init( &alliumV2_ctx.blake );
    sph_blake256( &alliumV2_ctx.blake, endiandata, 64 );
